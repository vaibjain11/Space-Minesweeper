...
export class SpaceMinesweeper extends gfx.GfxApp
{
    ...
    update(deltaTime: number): void 
    {
        ...
        for(let i=0; i < this.starfield.numParticles; i++)
        {
            const starPosition = this.starfield.particlePositions[i];                     //current position
            const starSpeed = 300 * this.starfield.particleSizes[i];      //speed relative to size of particle and time

           // ADD YOUR CODE HERE
           const starDistance = gfx.Vector2.multiplyScalar(moveDirection, -shipSpeed);
           //const distance = gfx.Vector2.subtract(this.mousePosition, this.ship.position);
           starDistance.multiplyScalar(starSpeed);
           starPosition.add(starDistance);
        ...
        this.mines.children.forEach((mine: gfx.Transform2) => {
            ...
            // ADD YOUR CODE HERE
            const mineDistance = gfx.Vector2.multiplyScalar(moveDirection, -shipSpeed);
            mine.position.add(mineDistance);
        ...
        this.lasers.children.forEach((laser: gfx.Transform2) => {
            ...
            if(laser.position.x < -1.1 || laser.position.x > 1.1 || laser.position.y > 1.1 || laser.position.y < -1.1) {
                laser.remove();
            }
    ...
    {
        const laserSpawn = new ShapeInstance(this.laser);
        laserSpawn.lookAt(this.mousePosition);
        this.lasers.add(laserSpawn);
    }
    ...
    private checkForMineCollisions(): void
    {
        for(let i=0; i < this.mines.children.length; i++)
        {
            for(let j=i+1; j < this.mines.children.length; j++)
            {
                ...
                // ADD YOUR CODE HERE
                if(mine1.intersects(mine2, gfx.IntersectionMode2.BOUNDING_CIRCLE)) {
                    mine1.explode();
                    mine2.explode();
                }
